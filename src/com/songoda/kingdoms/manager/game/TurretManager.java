...
import com.songoda.kingdoms.events.KingdomTurretBreakEvent;
...
public class TurretManager extends Manager {
  protected TurretManager(Plugin plugin){
	...
	//initTurrets();
	int turrets = TurretType.values().length;
	int disabled = 0;
	for(TurretType tt : TurretType.values()){
	  if(!tt.isEnabled() || tt.toString().startsWith("MINE")){
		disabled++;
	  }
	}
	if(disabled >= turrets){
...
/*
  ...
  public Collection<Entity> getNearbyChunkEntities(Chunk chunk, int range){
	Collection<Entity> mobs = new ArrayList();
  ...
  public void dispenserEvent(BlockDispenseEvent e){
	...
	BlockFace targetFace = ((org.bukkit.material.Dispenser) d.getData()).getFacing();
	if(isTurret(e.getBlock().getRelative(targetFace))){
	  e.setCancelled(true);
	}
  }
  ...
  public void onBlockUnderPressurePlateBreak(BlockBreakEvent event){
	Block t = event.getBlock().getRelative(0, 1, 0);
	if(t.getType().toString().contains("PLATE") && isTurret(t)){
	  SimpleLocation loc = new SimpleLocation(t.getLocation());
	  Land land = GameManagement.getLandManager().getOrLoadLand(loc.toSimpleChunk());
	  Turret tur = land.getTurret(loc);
	  tur.breakTurret();
	}
  }
...
//    }
  ...
  public void onTurretArrowHit(EntityDamageByEntityEvent event){
	if(event.getDamager().hasMetadata(TurretUtil.META_DAMAGE)){
