...
public class TurretUtil {
  ...
  //async it.
  public static boolean canBeTarget(Turret turret, Player target){

	if(target.isDead() || !target.isValid()) return false;
	if(GameManagement.getChampionManager().isChampion(target)) return false;
	if(!turret.getLoc().getWorld().equals(target.getWorld().getName())) return false;
	if(target.getLocation().distanceSquared(turret.getLoc().toLocation()) >
		Math.pow(turret.getType().getRange(), 2)) return false;
	TurretType type = turret.getType();
	UUID kingdomOwner = GameManagement.getLandManager().getOrLoadLand(turret.getLoc().toSimpleChunk()).getOwnerUUID();
	...
	if(kp.isTemp()) return false;
	if(kp.isAdminMode()) return false;
	if(kp.isVanishMode()) return false;
	if(kp.getPlayer().getGameMode() != GameMode.SURVIVAL &&
		kp.getPlayer().getGameMode() != GameMode.ADVENTURE) return false;

	if(type.getTargetTypes().contains(TurretTargetType.ALLY_PLAYERS)){
	  if(playerKingdom == null) return false;
	  if(kingdomOwner.equals(playerKingdom.getKingdomUuid())) return true;
	  ...
	  if(kingdomOwner.equals(playerKingdom.getKingdomUuid())) return false;
	  return !landOwner.isAllianceWith(playerKingdom);
	}
	return false;
  }
  ...
  public static boolean canBeTarget(Kingdom kingdom, Entity target){
	...
	if(target.isDead() || !target.isValid()) return false;
	if(ExternalManager.isCitizen(target)) return false;
	Kingdom playerKingdom = null;
	if(GameManagement.getChampionManager().isChampion(target)) return false;
	...
	if(playerKingdom == null) return true;
	if(kingdom.getKingdomName().equals(playerKingdom.getKingdomName())) return false;
  ...
  public static void shootArrow(Kingdom shooter, Location target, Location origin, boolean crit, boolean fire, double damage){
	...
	arrow.setMetadata(META_SHOOTER, new FixedMetadataValue(Kingdoms.getInstance(), shooter.getKingdomName()));
	if(crit)
	  arrow.setCritical(crit);
	if(fire)
	  arrow.setFireTicks(Integer.MAX_VALUE);

	arrow.setMetadata(META_DAMAGE,
		new FixedMetadataValue(Kingdoms.getInstance(), "" + damage));
  ...
  public static void volley(Kingdom shooter, Location target, Location origin){
	...
	while(i < 2){
	  ...
	  snowball.setMetadata(META_SHOOTER, new FixedMetadataValue(Kingdoms.getInstance(), shooter.getKingdomName()));
	  snowball.setMetadata("flamesnowballs", new FixedMetadataValue(Kingdoms.getInstance(), "fireballs"));
	}
