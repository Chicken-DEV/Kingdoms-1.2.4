...
public class KCommandMod extends KCommandBase {
	...
	public boolean canExecute(CommandSender sender){
		...
		if(sender.hasPermission("kingdoms.mod")){
			return true;
		}
	...
	public void executeCommandUser(Player user, Queue<String> args) {
		...
		//check if player is already a mod
		...
		if(!kp.getRank().isHigherOrEqualTo(Rank.KING)){
			Kingdoms.getInstance();
			kp.sendMessage(Kingdoms.getLang().getString("Misc_Rank_Too_Low", kp.getLang()).replaceAll("%player%", "King"));
			return;
		}
		...
		if(target.getRank().isHigherOrEqualTo(Rank.MODS)){
			Kingdoms.getInstance();
			kp.sendMessage(Kingdoms.getLang().getString("Command_Mod_Member_Already_Mod", kp.getLang()));
			return;
		}
		
		Kingdoms.getInstance();
		kp.sendMessage(ChatColor.GREEN + Kingdoms.getLang().getString("Command_Mod_Broadcast", kp.getLang()).replaceAll("%player%", target.getName()));
		kp.getKingdom().sendAnnouncement(null, Kingdoms.getLang().getString("Command_Mod_Broadcast", kp.getLang()).replaceAll("%player%", target.getName()), true);
		target.setRank(Rank.MODS);
	}
	
	@Override
	public String getDescription(String lang) {
		return Kingdoms.getLang().getString("Command_Help_Mod", lang);
	}
