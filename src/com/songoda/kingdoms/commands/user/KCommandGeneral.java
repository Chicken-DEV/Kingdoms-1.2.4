...
public class KCommandGeneral extends KCommandBase {
	...
	public boolean canExecute(CommandSender sender){
		...
		if(sender.hasPermission("kingdoms.general")){
			return true;
		}
	...
	public void executeCommandUser(Player user, Queue<String> args) {
		//check if king
		//check if setting king as general
		//check if player in in the kingdom
		//check if player is already a general
		...
		if(target.getRank().isHigherOrEqualTo(Rank.GENERALS)){
			Kingdoms.getInstance();
			kp.sendMessage(Kingdoms.getLang().getString("Command_General_Member_Already_General", kp.getLang()));
			return;
		}
		
		Kingdoms.getInstance();
		kp.sendMessage(ChatColor.GREEN + Kingdoms.getLang().getString("Command_General_Broadcast", kp.getLang()).replaceAll("%player%", target.getName()));
		kp.getKingdom().sendAnnouncement(null, Kingdoms.getLang().getString("Command_General_Broadcast", kp.getLang()).replaceAll("%player%", target.getName()), true);
		target.setRank(Rank.GENERALS);
	}
	
	@Override
	public String getDescription(String lang) {
		return Kingdoms.getLang().getString("Command_Help_General", lang);
	}
